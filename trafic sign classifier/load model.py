...
print "length of data testData[labels] " + str(len(testData["labels"]))
X_test = testData["features"]
# X_train_ini = np.reshape(X_train,(len(trainingData["features"]),32,32,3)).astype(np.float32)#*255
X_test_ini = X_test[:,:,:]
X_test_ini = np.reshape(X_test,(len(testData["features"]),32,32,3)).astype(np.float32)#*255
y_test = testData["labels"]
y_test_ini = np.squeeze(y_test[:,:,:]).astype(np.int32)
print "length, type and shape of x train features",len(X_test_ini),X_test_ini.dtype ,X_test_ini.shape
print "length, type and shape of y train labels",len(y_test_ini), y_test_ini.dtype ,y_test_ini.shape
print "y sample class label" ,y_test_ini[500]


def main():
	X_train_sub, y_train_sub, X_val, y_val = split_dataset(X_test_ini,y_test_ini)
	# EPOCHS = 25
	# BATCH_SIZE = 128
	learning_rate = 0.001
	start_time = time()
